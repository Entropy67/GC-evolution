...
class Bcell:  
    def __init__(self, gc, E0, xb, f=None, founder_id = None):
        self.gc = gc
        self.alive = True
        ...
        self.w = 0 ### fitness
        self.id = founder_id ### founder B cell id
        if f is None:
            self.f = self.gc.f
        else:
            self.f = f
    ...
    def divide(self, num_children=2):
        ...
        for i in range(num_children):
            if self.gc.inheritance_f:
                soni = Bcell(self.gc, self.E, self.xb, self.f, self.id)
            else:
    ...
    def extract(self, Ea, xa):
        ## determine force
        if self.sigma_f > 0 and not self.gc.inheritance_f:
            #f = max(0, self.gc.f*(1 + np.random.normal(0, self.gc.sigma_f)))
            #print("use Gaussian")
            self.f = max(0, self.gc.f*(1 + 2*self.sigma_f*(np.random.uniform() - 0.5)))
...
class GC:
    ...
    def setup(self):
        ...
        self.tracker_size = self.prm["track_size"]
        ...
        for i in range(self.N0):
            if self.initCond == "fixed":
                ...
                fmax = self.f*(1 + self.sigma_f)
                fmin = self.f*(1-self.sigma_f)
                if self.sigma_f ==0:
                    f_min = f - 0.5
                    fmax = f + 0.5
                bcell = Bcell(self, eb, xb, f, int(self.tracker_size*(f-fmin)/(fmax-fmin)))
                
            self.addAliveBcell(bcell)
            
        ### tracking B cell clone
        self.tracker = tracker.Tracker(size=self.tracker_size + 1, 
                                       qty_name = ["xb", "E", "f", "eta", "w" , "tau", "log_tau"],
                                       gc = self)
        self.checkN()
        self.N_ave = self.N
