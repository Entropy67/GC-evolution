...
the force oscillates over generations
"""
...
prm_list = {
    ...
    "f":0,  # const force
    "df": 0, ## force increasing step size
    "Tf": 50, ## force period
    "sigma_f": 0, ### noise in force magnitude
    "eta0": 0.5, 
    "tm": 300,
...
class manyRun:
    def __init__(self, prm=prm_list):
        ...
        self.saved_qty = ["e", ### affinity at tmax
                          ...
                          "tauStd",  ### lifetime std
                          "log_tau", ### log10 of tau
                          "log_tauStd", ### std of log10 of tau
                          "Qave", ### time-averaged binding quality
                          "QS", ### entropy of tau
                          "Qp", ### plasma cell binding quality
                          "fStd", ### force diversity
                          "fS", ### entropy of force
                          "fAve", ### average force
    ...
    def run(self, output=False):
        ...
        if success_count == 0:
            ...
            if self.output:
                print("many run: summary: surv_prob = {2:.4f}\t, Eb = {3:.3f}\t, tau={4:.3f}".format(success_count, dead_count, self.surv_prob, np.mean(self.dataset["e"]), np.log10(np.mean(self.dataset["tau"])/self.gc.dataset["tau"][0])))
...
class Bcell:  
    def __init__(self, gc, E0, xb, f=None):
        self.gc = gc
        self.alive = True
        ...
        if f is None:
            ...
            ## determine force
            if self.gc.sigma_f > 0 and not self.gc.inheritance_f:
                #f = max(0, self.gc.f*(1 + np.random.normal(0, self.gc.sigma_f)))
                #print("use Gaussian")
                self.f = max(0, self.gc.f*(1 + 2*self.gc.sigma_f*(np.random.uniform() - 0.5)))
                #f = rd.random() * self.gc.f
            else:
                self.f = self.gc.f
        else:
            self.f = f
    ...
    def divide(self, num_children=2):
        ...
        for i in range(num_children):
            if self.gc.inheritance_f:
                soni = Bcell(self.gc, self.E, self.xb, self.f)
            else:
    ...
    def extract(self, Ea, xa):
        ...
        if FEEDBACK_XB:
            ...
            self.eta = self.gc.Ag_extract(Ea, self.E, xa,  self.xb, f)
        else:
            ## the tether bond length is fixed at gc.xb1
            self.eta = self.gc.Ag_extract(Ea, self.E, self.gc.xb1,  self.xb, f) ### fixing the tether stiffness during evolution
        self.mature=True
        ## convert to fitness
...
class GC:
    def __init__(self, f0=10.0, prm=prm_list):
        ...
        self.qty_name = ["e", ### GC B cell affinity mean
                         ...
                         "Q", #### binding quality log10(tau/tau0)
                         "QStd", #### std of binding qualityy
                         
                         "Qave", ### time-averaged binding quality
                         
                         "QS", #### shannon entropy for binding quality
                         "tauF", ### GC B cell lifetime under force
                         "fAve", ###average force
                         "fStd", ### force variability
                         "fS", ### force entropy
                         "tau0", ### initial lifetime
                         ...
                         "mat_rate",
                         "f",
                         "Ntot", ### accumulated number of B cells
                        ]
    ...
    def load_prm(self, prm):
        ...
        self.f = prm["f"]         ### force 
        self.f0 = prm["f"]        #### maximal force
        self.sigma_f = prm["sigma_f"] ### force fluctuation
        self.inheritance_f = prm["inheritance_f"]
        
        self.Tf = prm["Tf"]       ### periodic force, period 
        self.scheme = prm["scheme"]
    ...
    def setup(self):
        ...
        for i in range(self.N0):
            if self.initCond == "fixed":
                bcell = Bcell(self, self.Eb, self.xb2)
            else:
                eb = self.Eb + np.random.normal(0, self.prm["sigma_eb"])
                xb = max(0.5, self.xb2 + np.random.normal(0, self.prm["sigma_xb"]))
                f = max(0, self.f*(1 + 2*self.sigma_f*(np.random.uniform() - 0.5)))
                bcell = Bcell(self, eb, xb, f)
        ...
        if self.initCond == "fixed":
            self.int_eb, self.init_xb, self.init_f = self.Eb, self.xb2, self.f
            
        else:
            self.init_eb = [bcell.E for bcell in self.agents]
            self.init_xb = [bcell.xb for bcell in self.agents]
            self.init_f = [bcell.f for bcell in self.agents]
            self.init_tau = [lifetime_linear_cubic(bcell.E, bcell.xb) for bcell in self.agents]
        self.tau0 = lifetime_linear_cubic(self.Eb, self.xb2)
    ...
    def update_force(self):
        if self.scheme == "sin":
            self.f = self.f0 * ( 1 + np.sin(2*3.1416*self.t/self.Tf))/2
        elif self.scheme=="cos":
            self.f = self.f0 * ( 1 + np.cos(2*3.1416*self.t/self.Tf))/2
        elif self.scheme == "ramping":
            self.f += self.df
        elif self.scheme == "step":
            self.f = self.f0 * ((self.t // self.Tf + 1) % 2)
        else:
            self.f = self.f0
        return
    
    ############# main fuctions ####################
    def Ag_extract(self, Ea, Eb, xa, xb, f):
        if self.useBell or self.potential == "bell":
            eta0, flag = extRatio(xa, xb, Ea, Eb, 0, output=False)
            if not flag and self.useSim:
                eta0 = self.read_eta_from_record(Ea, Eb, xa, xb, 0)
            #tau0 = 1/eta0 - 1  ###tau0 = 0.5*np.exp(Ea-Eb)
            eta = 1.0/(1.0+(1.0/eta0 - 1)*np.exp(f*(xb-xa)/4.012))
        
        else:
            if self.potential=="cusp":    
                eta, flag = extRatio(xa, xb, Ea, Eb, f, output=False)

            elif self.potential=="linear-cubic":
                eta, flag, barrier_height = extRatio_linear_cubic(xa, xb, Ea, Eb, f, output=False, Eb_min=self.prm["Eb_min"])
            if not flag and not self._warned and self.output and False:
                print("*GC warning: low barrier! Ea={0:.1f}, Eb={1:.1f}, dE={2:.2f}, xa={5:.2f}, xb={4:.2f}, f={3:.3f}".format(Ea, Eb, barrier_height, f, xb, xa))
                self._warned=True

            if not flag and self.useSim:
                if self.output and not self._used_sim_already:
                    if eta is None and self.output and False:
                        print("*start sim @t={0:.0f}, F={1:.2f}, eta_theory=None".format(self.t, f), end=", ")
                        pass
                    elif self.output and False:
                        print("*start sim @t={0:.0f}, F={1:.2f}, eta_theory={2:.3f}".format(self.t, f, eta), end=", ")
                eta = self.read_eta_from_record(Ea, Eb, xa, xb, f)
    ...
    def collectData(self):
        ...
        log_tau = np.log10(taulist) - np.log10(self.tau0)
        self.append("tau", tau_mean)
        self.append("tauVar", tau_var)
        self.append("tauStd", np.sqrt(tau_var))
        self.append("log_tau", self._get_mean(log_tau))
        self.append("log_tauStd", np.sqrt(self._get_var(log_tau)))
        self.append("Q", self._get_mean(log_tau))
        
        self.append("QStd", np.sqrt(self._get_var(log_tau)))
        
        current_plasma_tau = self.getCurrentPlasmaLifetimeList()
        current_plasma_Q = np.log10(current_plasma_tau ) - np.log10(self.tau0)
        self.append("Qp", self._get_mean(current_plasma_Q))
        
        if len(self.dataset["Qave"])>0:
            Qtot = self.dataset["Qave"][-1]*self.dataset["Ntot"][-1] + np.sum(log_tau)
            Ntot = self.dataset["Ntot"][-1] + len(log_tau)
            self.append("Qave", Qtot/Ntot)
            self.append("Ntot", Ntot)
        else:
            if len(log_tau)>0:
                self.append("Qave", self._get_mean(log_tau))
                self.append("Ntot", len(log_tau))
            
        self.append("QS", self.getShannonEntropy(log_tau, np.linspace(-1, 3, 21)))
        ...
        ### record force distirbution
        flist = self.getCurrentForceList()
        self.append("fAve", self._get_mean(flist))
        self.append("fStd", np.sqrt(self._get_var(flist)))
        self.append("fS", self.getShannonEntropy(flist, np.linspace(0, 20, 21)))
    ...
    def lightZoneStep(self, test=False):
        ...
        self.extract_antigen()
        return
    
    def extract_antigen(self):
    ...
    def getCurrentFList(self):
        flist = []
        for bcell in self.agents:
            if bcell.alive:
                flist.append(bcell.f)
        return flist
    ...
    def getShannonEntropy(self, alist, bins):
        tot_count = len(alist)
        p, _ = np.histogram(alist, bins=bins)
        
        S = 0
        for pi in p:
            if pi>0:
                S += - (pi/tot_count) * np.log(pi/tot_count)
        return S
...
def extRatio_linear_cubic(xb1, xb2, e10, e20, f0, output=True, Eb_min=3):
    """
...
def lifetime_linear_cubic(eb, xb, f0=0):
    kT = 4.012
    if f0==0:
        fb = 3*eb*kT/(2*xb)
        k = 4*(fb/xb)*0.001 ## nN/nm
        return 2*3.1415*np.exp(eb)/k
    else:
        fb = 3*eb*kT/(2*xb)
        Ef = eb*(1-f0/fb)**(3/2)
        k = 4*(fb/xb)*0.001 * np.sqrt(1 - f0/fb)
        return 2*3.1415*np.exp(Ef) / k
